...
Любой реальный проект - это поиск баланса между «как правильно» и «как можно с учётом ресурсов». В этой работе столкнулся с несколькими ограничениями и сделал осознанный выбор в пользу практичности. Вот ключевые решения, которые определили итоговую архитектуру.

#### Только один публичный IP-адрес

При попытке развернуть Bastion, Grafana, Kibana и балансировщик с отдельными публичными IP быстро упёрся в квоту Yandex Cloud: больше одного IP в минуту создать нельзя. Запрос на увеличение квоты отклонили - это внутреннее ограничение платформы. Последовательное создание с паузами не помогало, а статические IP требовали ручного вмешательства и не решали проблему скорости.

В итоге оставил публичный IP только для Bastion, а все остальные сервисы спрятали в приватные подсети. Доступ к ним идёт через Nginx-прокси на Bastion. Так мы обошли квоту, сократили количество публичных точек входа и даже повысили безопасность. Минус - Grafana и Kibana теперь не доступны напрямую из интернета, но для внутренних сервисов это даже предпочтительнее.

#### Балансировка без управляемого ALB

По заданию нужна балансировка между веб-серверами. Можно было бы взять готовый Application Load Balancer от Yandex, но он требует отдельного публичного IP (уже решил, что лишних IP не будет) и стоит денег. Альтернативы - Network Load Balancer или HAProxy на отдельной ВМ - тоже не подходили.

Поставил Nginx на Bastion и сделал его одновременно и прокси, и балансировщиком. Он раздаёт трафик на два веб-сервера, а заодно проксирует запросы к Grafana и Kibana. Получилось централизованное решение без дополнительных затрат. Плата за это я сам отвечаю за его работу: обновления, мониторинг, отказоустойчивость. Но зато имеем полный контроль и понимание того, как всё устроено.
...
Функциональность не пострадала - все веб-интерфейсы доступны. При этом сервисы не светятся в интернете напрямую, что хорошо с точки зрения безопасности. Да, это отклонение от задания, но оно обосновано и очень часто встречается на практике (reverse proxy для внутренних приложений).

#### Прерываемые ВМ для экономии

Облачные ресурсы стоят денег, а бюджет не резиновый. Могли бы взять обычные ВМ с гарантированной доступностью, но они дороже. Вместо этого выбрал прерываемые инстансы (`preemptible = true`) - они дают скидку до 70%.
...
Я решил, что для нашей задачи проще и дешевле развернуть всё в Docker на собственных ВМ. Prometheus, Grafana, Elasticsearch, Kibana - все работают в контейнерах, управление через Ansible. У нас полный доступ к конфигам, легко отлаживать, и не привязаны к конкретному облачному вендору.
...
Я выбрал Docker - все сервисы запускаются из официальных образов. Это даёт единообразие окружения, лёгкость в обновлении и переносимость между средами. Единственный минус - небольшой оверхед по производительности и необходимость разбираться в контейнерах, но сегодня это стандарт де-факто, так что вложения времени окупаются.
---
