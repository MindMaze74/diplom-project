...
В ходе проектирования и реализации инфраструктуры был принят ряд решений, которые отступают от «идеальной» архитектуры, но являются обоснованными в условиях ограничений облачного провайдера, бюджета и учебных целей проекта. Ниже приведены ключевые компромиссы с подробным обоснованием.
...
- **Компромисс**: Потеряна прямая публичная доступность Grafana и Kibana, но это не критично для курсовой работы, а для реальных проектов часто является предпочтительным с точки зрения безопасности.
...
- **Результат**: Балансировка работает, не требует доп. затрат и публичных IP. Nginx на Bastion также выступает прокси для Grafana и Kibana, что делает его центральным элементом маршрутизации.  
- **Компромисс**: Балансировщик не является управляемым сервисом Yandex Cloud, поэтому мы берём на себя его администрирование (мониторинг, обновление, отказоустойчивость). Однако для учебного проекта это допустимо и даже полезно, так как демонстрирует навыки настройки прокси и балансировки вручную.
...
- **Компромисс**: Отступление от буквы задания, но с обоснованием (квота, безопасность). В реальных проектах часто используют аналогичный подход (reverse proxy для внутренних сервисов).

#### 4. Использование прерываемых (preemptible) ВМ

- **Проблема**: Бюджет курсовой работы ограничен, необходимо минимизировать затраты на облачные ресурсы.  
- **Варианты решения**:  
...
- **Компромисс**: Прерываемые ВМ могут быть остановлены в любой момент (максимальное время жизни — 24 часа). Это означает, что инфраструктура не подходит для продакшн-нагрузок, но для курсовой работы, где важна демонстрация, а не постоянная работа, это приемлемо. При необходимости сервисы будут автоматически перезапущены политикой Docker.
...
- **Варианты решения**:  
  - Использовать Managed Databases для Prometheus (адаптер PostgreSQL) – дорого и избыточно для учебного проекта.  
...
- **Компромисс**:  
  - **Нет автоматического масштабирования** – при росте нагрузки пришлось бы вручную увеличивать ресурсы ВМ или добавлять новые.  
  - **Нет автоматического failover (отказоустойчивости)** – если какая-то ВМ упадёт, сервис станет недоступен до ручного перезапуска (или автоматического перезапуска Docker-контейнера). В отличие от управляемых сервисов, здесь нет встроенных механизмов репликации и автоматического переключения на здоровую ноду.  
  - Это означает, что инфраструктура не гарантирует 99.9% доступности, но для курсовой работы это допустимо. Проект демонстрирует понимание архитектуры, а не обеспечивает промышленную надёжность. В реальных условиях мы бы использовали Managed Kubernetes и Managed Databases, но в рамках курсовой работы такой подход был бы избыточным.
...
- **Компромисс**: Docker добавляет небольшой оверхед по производительности и требует навыков работы с контейнерами, но в современных проектах это стандарт. Для учебных целей это оправдано.
